...
#
# This lightweight CNN model includes:
# - 4 convolutional layers with ReLU activation and max pooling.
# - Adaptive average pooling to handle variable input sizes.
# - A classifier with dropout for regularization:
#   * Dropout helps prevent overfitting by randomly deactivating neurons during training.
#   * Note: Dropout is active during training but turned off during validation/testing.
#   * This explains the difference in accuracy between training and validation phases later on.
# - 11 output neurons for the 11 font classes.
##################################################
# CNN model definition
class LightCNN(nn.Module):  # A lightweight CNN model
...
#
# This function creates an instance of LightCNN with the
...
# - Suggests hyperparameters to test (learning rate, batch size, optimizer)
# - Creates and trains a model with these parameters for a few epochs
# - Returns validation loss for Optuna to minimize
# - Uses early stopping to avoid wasting time on poor configurations
##################################################
def objective(trial):
    ...
    print(
        f"Starting hyperparameter tuning trial with lr={lr}, batch_size={batch_size}, optimizer={optimizer_name}"
    )
...
def main():
    ...
    csv_file = "data/pages_processed.csv"
    ...
    #
    # Includes:
    # - Resizing images to 224x224 pixels (standard for many CNNs)
    # - Converting to PyTorch tensors
    # - Normalizing pixel values to [-1,1] range
    ##################################################
    # Reduce image size for faster processing (512x512 to 224x224) as GPU memory is limited
...
if __name__ == "__main__":
    ...
    main()
    # Note: The script assumes the presence of the CSV file (pages_processed.csv) with image paths and labels
